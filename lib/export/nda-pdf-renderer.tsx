// lib/export/nda-pdf-renderer.tsx
// React-PDF document component for the NDA Agreement.
// SELF-CONTAINED — no imports from any other document-specific renderer.
// Edit this file freely; changes here will NOT affect Offer Letters, Internship
// Letters, Salary Slips, Experience Letters, or Appointment Letters.
//
// ── NOTE ──────────────────────────────────────────────────────────────────────
// This file was created as an independent copy of the shared renderer with the
// component renamed to NDAPdfDocument. The USER will update this file with the
// previous NDA-specific styles/layout when provided.
// ─────────────────────────────────────────────────────────────────────────────
...
try {
  ...
  console.error('[nda-pdf-renderer] Failed to load logo image to base64:', err)
}
// ─── Brand constants ──────────────────────────────────────────────────────────
const BRAND = {
  sageLight:   '#b5bbb2',
...
const styles = StyleSheet.create({
  ...
  tableCell: {
    flex: 1, fontSize: 10, lineHeight: 1.4, color: BRAND.bodyText,
    paddingVertical: 5, paddingHorizontal: 7,
    borderRightWidth: 0.5, borderRightColor: BRAND.divider,
  },
  tableCellLast: {
    flex: 1, fontSize: 10, lineHeight: 1.4, color: BRAND.bodyText,
    paddingVertical: 5, paddingHorizontal: 7,
  ...
  tableHeaderCell: {
    flex: 1, fontSize: 10, fontFamily: 'Helvetica-Bold', lineHeight: 1.4,
    color: BRAND.headingText, paddingVertical: 5, paddingHorizontal: 7,
    borderRightWidth: 0.5, borderRightColor: BRAND.divider,
  },
  tableHeaderCellLast: {
    flex: 1, fontSize: 10, fontFamily: 'Helvetica-Bold', lineHeight: 1.4,
  ...
  footerDarkBand: {
    height: 40, backgroundColor: BRAND.sageDark,
...
function InlineText({ segs, baseStyle }: { segs: Segment[]; baseStyle?: PDFStyle }) {
  return (
    <Text style={baseStyle}>
      {segs.map((s, i) => (
        <Text key={i} style={segmentStyle(s)}>{s.text}</Text>
      ))}
    </Text>
  )
}
            ...
            {cells.map((cell, colIdx) => {
              ...
              return (
                <View key={colIdx} style={cellStyle}>
                  <InlineText segs={segs} baseStyle={isHeader ? { fontFamily: 'Helvetica-Bold' } as PDFStyle : undefined} />
                </View>
              )
            })}
          </View>
...
function markdownToElements(markdown: string): React.ReactElement[] {
  ...
  while (i < lines.length) {
    ...
    if (isTableRow(trimmed)) {
      ...
      while (i < lines.length && isTableRow(lines[i].trim())) { tableLines.push(lines[i]); i++ }
      const rows: string[][] = []
      for (const tl of tableLines) { if (isSeparatorRow(tl)) continue; rows.push(splitTableRow(tl)) }
    ...
    if (numMatch) {
      els.push(
        <View key={i} style={styles.listRow} wrap={false}>
          ...
          <InlineText segs={parseInline(numMatch[2])} baseStyle={styles.listText} />
        </View>
      )
      i++; continue
    }
...
export interface NDADocumentProps {
  companyName: string
  documentTitle: string
  markdownContent: string
  dateStr: string
}
export function NDAPdfDocument({ companyName, documentTitle, markdownContent }: NDADocumentProps) {
  return (
